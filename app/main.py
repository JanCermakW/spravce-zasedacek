...
def create_booking(data: BookingCreate, session: Session = Depends(get_session)):
    ...
    try:
        BookingService.validate_booking_attendees(data.attendees)
        BookingService.validate_capacity(room, data.attendees)
        BookingService.validate_times(data.start_time, data.end_time)
        BookingService.validate_working_days(data.start_time)
        BookingService.validate_user_limit(session, data.user_id)
        BookingService.check_availability(
            session, room.id, data.start_time, data.end_time
        )
